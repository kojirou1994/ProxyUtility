...
//            }
        case .urlsession:
            ...
            config.setProxy(proxyType: .socks5, host: "127.0.0.1", port: socks5Port)
            config.urlCache = nil
            let session = URLSession(configuration: config)
            let startDate = Date()
            let req = URLRequest.init(url: testURL, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: 10)
            return try session.syncDataTask(request: req) { data, response, error in
                if let e = error {
                    throw e
                } else {
